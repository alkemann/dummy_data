...
class Lorem extends \dummy_data\models\Data {
	 ...
	 * @return string
	 */
	public static function word($options = array()) {
	 ...
	 * @author Caius Durling
	 */
	private static function words( $num=3 ) {
		$w = self::word_list();
		shuffle( $w );
	 ...
	 * @author Caius Durling
	 */
	public static function sentence( $options = array('max' => 8) ) {
		if (!isset($options['max'])) {
			$options['max'] = 8;
		}
		$words = static::words( rand(2, $options['max']) );
